...
nexttile(2)
title("Histidine Kinase $\boldsymbol{H}$", 'interpreter', 'latex')
xlabel('Time (min)')
...
nexttile(3)
title("Regulator Complex $\boldsymbol{R_pH}$", 'interpreter', 'latex')
xlabel('Time (min)')
ylabel('Concentration (\textmu M)')
hold on
...
nexttile(4)
title("Phosphorylated Kinase $\boldsymbol{H_p^\textbf{total}}$", 'interpreter', 'latex')
xlabel('Time (min)')
ylabel('Concentration (\textmu M)')
hold on
...
h1 = plot(ground_truth.t, ground_truth.moriginal(:, 5), ...
          '-.', 'LineWidth', 2, 'Color', colGTS);
plot(ground_truth.t, CI_true(:, 5, 1), '-.', 'LineWidth', 1.5, 'Color', coltrue);
plot(ground_truth.t, CI_true(:, 5, 2), '-.', 'LineWidth', 1.5, 'Color', coltrue);
h4 = patch([ground_truth.t flip(estimator.results_GTS.t)], [CI_true(:, 5, 1)' flip(CI_true(:, 5, 2))'], ...
           coltrue, 'EdgeAlpha', 0, 'FaceAlpha', .15, 'LineStyle', 'none');
       
h1 = plot(estimator.results_GTS.t, estimator.results_GTS.population(:, 5), ...
          '-.', 'LineWidth', 2, 'Color', colGTS);
plot(estimator.results_GTS.t, CI_GTS(:, 5, 1), '-.', 'LineWidth', 1.5, 'Color', colGTS);
plot(estimator.results_GTS.t, CI_GTS(:, 5, 2), '-.', 'LineWidth', 1.5, 'Color', colGTS);
h3 = patch([estimator.results_GTS.t flip(estimator.results_GTS.t)], [CI_GTS(:, 5, 1)' flip(CI_GTS(:, 5, 2))'], ...
           colGTS, 'EdgeAlpha', 0, 'FaceAlpha', .15, 'LineStyle', 'none');
       
h1 = plot(estimator.results_GMGTS.t_fine, estimator.results_GMGTS.population(:, 5), ...
          ':', 'LineWidth', 2, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine, CI_GMGTS(:, 5, 1), ':', 'LineWidth', 1.1, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine, CI_GMGTS(:, 5, 2), ':', 'LineWidth', 1.5, 'Color', colGMGTS);
...
%%
filestr = '../../writing/manuscript/figures/TCS.tex';
matlab2tikz(filestr)
