...
CI_GMGTS = quantile(estimator.results_GMGTS.fitted2(1:10:end, :, :), [.05 .95], 3);
...
hold on
beta_GMGTS = estimator.results_GMGTS.beta_fs(:, [3 4]);
beta_GTS = estimator.results_GTS.beta_fs(:, [3 4]);
beta_true = ground_truth.beta(:, [3 4]);
b_GMGTS = estimator.results_GMGTS.b_est([3 4]);
b_GTS = estimator.results_GTS.b_est([3 4]);
b_true = ground_truth.b([3 4]);
D_GMGTS = estimator.results_GMGTS.D_GTS([3 4], [3 4]);
D_GTS = estimator.results_GTS.D_GTS([3 4], [3 4]);
D_true = ground_truth.D([3 4], [3 4]);
...
h2 = patch([ground_truth.t flip(ground_truth.t)], [CI_true(:, 4, 1)' flip(CI_true(:, 4, 2))'], ...
           coltrue, 'EdgeAlpha', 0, 'FaceAlpha', .25, 'LineStyle', 'none');
...
h1 = plot(estimator.results_GMGTS.t_fine(1:10:end), estimator.results_GMGTS.population(1:10:end, 4), ...
          ':', 'LineWidth', 2, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 4, 1), ':', 'LineWidth', 1.1, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 4, 2), ':', 'LineWidth', 1.5, 'Color', colGMGTS);
h2 = patch([estimator.results_GMGTS.t_fine(1:10:end) flip(estimator.results_GMGTS.t_fine(1:10:end))], [CI_GMGTS(:, 4, 1)' flip(CI_GMGTS(:, 4, 2))'], ...
...
nexttile(3)
title("mRNA $\boldsymbol{m_2}$", 'interpreter', 'latex')
xlabel('Time (min)')
ylabel('Concentration (\textmu M)')
hold on

h = plot(estimator.results_GMGTS.t_data, ...
    reshape(estimator.results_GMGTS.traces(:, 5, 1:estimator.results_GMGTS.N), [], estimator.results_GMGTS.N), '-', 'LineWidth', .9);
set(h, {'color'}, num2cell(repmat(mean(parula(estimator.results_GMGTS.N), 2), 1, 3), 2));
            
h1 = plot(ground_truth.t, ground_truth.moriginal(:, 8), ...
          '-.', 'LineWidth', 2, 'Color', coltrue);
plot(ground_truth.t, CI_true(:, 8, 1), '-.', 'LineWidth', 1.5, 'Color', coltrue);
plot(ground_truth.t, CI_true(:, 8, 2), '-.', 'LineWidth', 1.5, 'Color', coltrue);
h2 = patch([ground_truth.t flip(ground_truth.t)], [CI_true(:, 8, 1)' flip(CI_true(:, 8, 2))'], ...
           coltrue, 'EdgeAlpha', 0, 'FaceAlpha', .25, 'LineStyle', 'none');
       
h1 = plot(estimator.results_GTS.t, estimator.results_GTS.population(:, 8), ...
          '-.', 'LineWidth', 2, 'Color', colGTS);
plot(estimator.results_GTS.t, CI_GTS(:, 8, 1), '-.', 'LineWidth', 1.5, 'Color', colGTS);
plot(estimator.results_GTS.t, CI_GTS(:, 8, 2), '-.', 'LineWidth', 1.5, 'Color', colGTS);
h2 = patch([estimator.results_GTS.t flip(estimator.results_GTS.t)], [CI_GTS(:, 8, 1)' flip(CI_GTS(:, 8, 2))'], ...
           colGTS, 'EdgeAlpha', 0, 'FaceAlpha', .15, 'LineStyle', 'none');
       
h1 = plot(estimator.results_GMGTS.t_fine, estimator.results_GMGTS.population(:, 8), ...
          ':', 'LineWidth', 2, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 8, 1), ':', 'LineWidth', 1.1, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 8, 2), ':', 'LineWidth', 1.5, 'Color', colGMGTS);
h2 = patch([estimator.results_GMGTS.t_fine(1:10:end) flip(estimator.results_GMGTS.t_fine(1:10:end))], [CI_GMGTS(:, 8, 1)' flip(CI_GMGTS(:, 8, 2))'], ...
           colGMGTS, 'EdgeAlpha', 0, 'FaceAlpha', .15, 'LineStyle', 'none');
...
nexttile(4)
title("Repressor $\boldsymbol{p_2}$", 'interpreter', 'latex')
xlabel('Time (min)')
ylabel('Concentration (\textmu M)')
hold on

h = plot(estimator.results_GMGTS.t_data, ...
    reshape(estimator.results_GMGTS.traces(:, 2, 1:estimator.results_GMGTS.N), [], estimator.results_GMGTS.N), '-', 'LineWidth', .9);
set(h, {'color'}, num2cell(repmat(mean(parula(estimator.results_GMGTS.N), 2), 1, 3), 2));
...
h2 = patch([ground_truth.t flip(ground_truth.t)], [CI_true(:, 2, 1)' flip(CI_true(:, 2, 2))'], ...
           coltrue, 'EdgeAlpha', 0, 'FaceAlpha', .25, 'LineStyle', 'none');
...
h1 = plot(estimator.results_GMGTS.t_fine, estimator.results_GMGTS.population(:, 2), ...
          ':', 'LineWidth', 2, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 2, 1), ':', 'LineWidth', 1.1, 'Color', colGMGTS);
plot(estimator.results_GMGTS.t_fine(1:10:end), CI_GMGTS(:, 2, 2), ':', 'LineWidth', 1.5, 'Color', colGMGTS);
h4 = patch([estimator.results_GMGTS.t_fine(1:10:end) flip(estimator.results_GMGTS.t_fine(1:10:end))], [CI_GMGTS(:, 2, 1)' flip(CI_GMGTS(:, 2, 2))'], ...
           colGMGTS, 'EdgeAlpha', 0, 'FaceAlpha', .15, 'LineStyle', 'none');
            
       
ylim(max(0, ylim))
legend([h(1) h2 h3 h4], {'Data', 'Truth', 'GTS', 'GMGTS'}, 'Location', 'southeast')
